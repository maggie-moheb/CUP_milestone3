...
class CEmit {
	...
	/*
	 * m_outstream.println("import java.lang.String;"); m_outstream.println(
	 * "import java.lang.System;"); m_outstream.println(
	 * "import java.io.BufferedReader;"); m_outstream.println(
	...
	private void print_details() {
		...
		for (i = 0; i < m_spec.m_row_map.length; ++i) {
			...
			if (null == accept) {
				...
				System.out.println(" [accepting, line " + accept.m_line_number + " <"
	...
	private void emit_states() throws java.io.IOException {
		...
		while (states.hasMoreElements()) {
			...
			m_outstream.println("\tprivate final int " + state + " = " + (m_spec.m_states.get(state)).toString() + ";");
			/* ++index; */
		}
	...
	private void emit_helpers() throws java.io.IOException {
		...
		m_outstream.println("\tprivate int[][] unpackFromString" + "(int size1, int size2, String st) {");
	...
	private void emit_driver() throws java.io.IOException {
		 ...
		 */
		m_outstream.println("\t\t\tyy_next_state = " + "yy_nxt[yy_rmap[yy_state]][yy_cmap[yy_lookahead]];");
		...
		if (NOT_EDBG) {
			...
			m_outstream.println("\t\t\tjava.lang.System.out.println(\"Accepting status = \"" + "+ yy_this_accept);");
			m_outstream.println(
					"\t\t\tjava.lang.System.out.println(\"Last accepting state = \"" + "+ yy_last_accept_state);");
			m_outstream.println("\t\t\tjava.lang.System.out.println(\"Next state = \"" + "+ yy_next_state);");
			m_outstream
					.println("\t\t\tjava.lang.System.out.println(\"Lookahead input = \"" + "+ ((char) yy_lookahead));");
		}
		 ...
		 * "= yy_state_dtrans[yy_lexical_state];"); m_outstream.println(
...
class CMakeNfa {
	...
	private void factor(CNfaPair pair) throws java.io.IOException {
		...
		if (m_lexGen.CLOSURE == m_spec.m_current_token || m_lexGen.PLUS_CLOSE == m_spec.m_current_token
			...
			if (m_lexGen.CLOSURE == m_spec.m_current_token || m_lexGen.OPTIONAL == m_spec.m_current_token) {
				start.m_next2 = end;
			}
	...
	private void term(CNfaPair pair) throws java.io.IOException {
		...
		if (m_lexGen.OPEN_PAREN == m_spec.m_current_token) {
			...
			if (false == (m_lexGen.ANY == m_spec.m_current_token || m_lexGen.CCL_START == m_spec.m_current_token
		...
		for (Enumeration e = m_spec.m_nfa_states.elements(); e.hasMoreElements();) {
			...
			for (int i = 0; i < ccls.length; i++)
				if (clsA.get(ccls[i])) // a split class
					if (nfa.m_edge == i || nfa.m_edge == CNfa.CCL && nfa.m_set.contains(i)) { // on
																								// A
																								// side
			...
			for (i = 0; i < group_count; ++i) {
				...
				for (j = 1; j < group_size; ++j) {
					...
					for (c = 0; c < m_spec.m_dtrans_ncols; ++c) {
						...
						if (goto_first != goto_next && (goto_first == CDTrans.F || goto_next == CDTrans.F
		...
		while (false == nfa_stack.empty()) {
			...
			if (CUtility.OLD_DUMP_DEBUG) {
				if (null != state.m_accept) {
					System.out.println("Looking at accepting state " + state.m_label + " with <"
			...
			if (null != state.m_accept && state.m_label < bunch.m_accept_index) {
				...
				if (CUtility.DEBUG) {
					...
					CUtility.ASSERT(CSpec.NONE == bunch.m_anchor || 0 != (bunch.m_anchor & CSpec.END)
	...
	void move(Vector nfa_set, SparseBitSet nfa_bit, int b, CBunch bunch) {
		...
		for (index = 0; index < size; ++index) {
			...
			if (b == state.m_edge || (CNfa.CCL == state.m_edge && true == state.m_set.contains(b))) {
				 ...
				 * bunch.m_nfa_bit.size()); System.out.println(
				 * "Reference index: " + state.m_next.m_label);
				 * System.out.flush();
				 */
				bunch.m_nfa_bit.set(state.m_next.m_label);
			}
...
class CAlloc {
	...
	static CNfaPair newNLPair(CSpec spec) {
		...
		if (spec.m_dtrans_ncols - CSpec.NUM_PSEUDO > 2029) {
			pair.m_start.m_next.m_set
					.add(2028); /* U+2028 is LS, the line separator */
			pair.m_start.m_next.m_set
					.add(2029); /* U+2029 is PS, the paragraph sep. */
		}
...
class CError {
	 ...
	 *******************************************************/
	static final String errmsg[] = { "Malformed regular expression.", "Missing close parenthesis.",
			"Too many regular expressions or expression too long.", "Missing [ in character class.",
			"^ must be at start of expression or after [.", "+ ? or * must follow an expression or subexpression.",
			"Newline in quoted string.", "Missing } in macro expansion.", "Macro does not exist.",
			"Macro expansions nested too deeply.", "JLex has not been successfully initialized.",
			"Unexpected end-of-file found.", "Undefined or badly-formed JLex directive.", "Internal JLex error.",
			"Unitialized state name.", "Badly formed macro definition.", "Syntax error.",
...
class CLexGen {
	...
	CLexGen(String filename) throws java.io.FileNotFoundException, java.io.IOException {
		...
		/* Open output stream. */
		m_outstream = new java.io.PrintWriter(new java.io.BufferedWriter(new java.io.FileWriter(filename + ".java")));
		if (null == m_outstream) {
	...
	private void userDeclare() throws java.io.IOException {
		...
		while (false == m_input.getLine()) {
			...
			if ('%' == m_input.m_line[0]) {
				...
				case '{':
					if (0 == CUtility.charncmp(m_input.m_line, 0, m_class_code_dir, 0, m_class_code_dir.length - 1)) {
						m_spec.m_class_code = packCode(m_class_code_dir, m_class_code_end_dir, m_spec.m_class_code,
				...
				case 'c':
					if (0 == CUtility.charncmp(m_input.m_line, 0, m_char_dir, 0, m_char_dir.length - 1)) {
						...
						break;
					} else if (0 == CUtility.charncmp(m_input.m_line, 0, m_class_dir, 0, m_class_dir.length - 1)) {
						m_input.m_line_index = m_class_dir.length;
						m_spec.m_class_name = getName();
						break;
					} else if (0 == CUtility.charncmp(m_input.m_line, 0, m_cup_dir, 0, m_cup_dir.length - 1)) {
				...
				case 'e':
					if (0 == CUtility.charncmp(m_input.m_line, 0, m_eof_code_dir, 0, m_eof_code_dir.length - 1)) {
						m_spec.m_eof_code = packCode(m_eof_code_dir, m_eof_code_end_dir, m_spec.m_eof_code,
								m_spec.m_eof_read, EOF_CODE);
						break;
					} else if (0 == CUtility.charncmp(m_input.m_line, 0, m_eof_value_code_dir, 0,
						...
						break;
					} else if (0 == CUtility.charncmp(m_input.m_line, 0, m_eof_throw_code_dir, 0,
				...
				case 'f':
					if (0 == CUtility.charncmp(m_input.m_line, 0, m_function_dir, 0, m_function_dir.length - 1)) {
						/* Set line counting to ON. */
						m_input.m_line_index = m_function_dir.length;
						m_spec.m_function_name = getName();
						break;
					} else if (0 == CUtility.charncmp(m_input.m_line, 0, m_full_dir, 0, m_full_dir.length - 1)) {
				...
				case 'i':
					if (0 == CUtility.charncmp(m_input.m_line, 0, m_integer_dir, 0, m_integer_dir.length - 1)) {
						/* Set line counting to ON. */
						m_input.m_line_index = m_integer_dir.length;
						m_spec.m_integer_type = true;
						break;
					} else if (0 == CUtility.charncmp(m_input.m_line, 0, m_intwrap_dir, 0, m_intwrap_dir.length - 1)) {
						/* Set line counting to ON. */
						m_input.m_line_index = m_integer_dir.length;
						m_spec.m_intwrap_type = true;
						break;
					} else if (0 == CUtility.charncmp(m_input.m_line, 0, m_init_code_dir, 0,
							m_init_code_dir.length - 1)) {
						m_spec.m_init_code = packCode(m_init_code_dir, m_init_code_end_dir, m_spec.m_init_code,
								m_spec.m_init_read, INIT_CODE);
						break;
					} else if (0 == CUtility.charncmp(m_input.m_line, 0, m_init_throw_code_dir, 0,
						...
						break;
					} else if (0 == CUtility.charncmp(m_input.m_line, 0, m_implements_dir, 0,
							m_implements_dir.length - 1)) {
						m_input.m_line_index = m_implements_dir.length;
						m_spec.m_implements_name = getName();
						break;
					} else if (0 == CUtility.charncmp(m_input.m_line, 0, m_ignorecase_dir, 0,
				...
				case 'n':
					if (0 == CUtility.charncmp(m_input.m_line, 0, m_notunix_dir, 0, m_notunix_dir.length - 1)) {
						/* Set line counting to ON. */
				...
				case 'p':
					if (0 == CUtility.charncmp(m_input.m_line, 0, m_public_dir, 0, m_public_dir.length - 1)) {
						/* Set public flag. */
				...
				case 'u':
					if (0 == CUtility.charncmp(m_input.m_line, 0, m_unicode_dir, 0, m_unicode_dir.length - 1)) {
				...
				case 'y':
					if (0 == CUtility.charncmp(m_input.m_line, 0, m_yyeof_dir, 0, m_yyeof_dir.length - 1)) {
						...
						m_spec.m_yyeof = true;
						break;
					} else if (0 == CUtility.charncmp(m_input.m_line, 0, m_yylex_throw_code_dir, 0,
		...
		if ('<' == m_input.m_line[m_input.m_line_index]) {
			...
			while (true) {
				...
				while (false == CUtility.isspace(m_input.m_line[m_input.m_line_index])
						&& ',' != m_input.m_line[m_input.m_line_index] && '>' != m_input.m_line[m_input.m_line_index]) {
					++m_input.m_line_index;
	...
	private void saveMacro() {
		...
		in_ccl = false;
		saw_escape = false;
		while (false == CUtility.isspace(m_input.m_line[elem]) || true == in_quote || true == in_ccl
				|| true == saw_escape) {
	...
	int advance() throws java.io.IOException {
		...
		/*
		 * if (m_input.m_line_index > m_input.m_line_read) { System.out.println(
		 * "m_input.m_line_index = " + m_input.m_line_index);
		...
		if (CUtility.FOODEBUG) {
			System.out.println("Lexeme: " + m_spec.m_lexeme + "\tToken: " + m_spec.m_current_token + "\tIndex: "
					+ m_input.m_line_index);
		}
	...
	private void details() {
		 ...
		 * "\t** Accept States and Anchor Vector **"); print_accept(); }
		 */
	}
			...
			if (null == m_spec.m_accept_vector && null == m_spec.m_anchor_array) {
				if (null == dtrans.m_accept) {
					...
					if (CSpec.NONE != dtrans.m_anchor) {
						System.out.print(" Anchor: " + ((0 != (dtrans.m_anchor & CSpec.START)) ? "start " : "")
				...
				if (null == accept) {
					...
					if (CSpec.NONE != m_spec.m_anchor_array[i]) {
						System.out.print(" Anchor: " + ((0 != (m_spec.m_anchor_array[i] & CSpec.START)) ? "start " : "")
